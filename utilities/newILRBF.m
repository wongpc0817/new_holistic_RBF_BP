...
function network = newILRBF(features,labels,trainingTarget,config)
...
while true
    ...
    if mod(errorCount,100)==0
        logbook.write("Error of round %d : %f \n",errorCount,error);
        fprintf("Error of round %d : %f \n",errorCount,error);
    ...
    if errorCount > 10
        if abs(errorRecord(end)-errorRecord(end-1)) <= 5e-10 || errorCount == config.epoch
            break
        end
        if error<0.0001
            break
